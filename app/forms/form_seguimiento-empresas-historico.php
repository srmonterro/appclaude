...
<!--     <div style="display:none" id="confirmacion" class="inv alert alert-success">Progreso guardado. Acuérdate de ir actualizándolo a medida que el alumno va avanzando en la formación.</div>
	...
	<ol class="breadcrumb">
      <li>Seguimiento</li>
      <li class="active">Seguimiento Empresas Histórico </li>
	</ol>

	<form role="form" action="functions/exportarvcf_seguimientoe.php" method="post" id="form-seguimientoeh">
		...
		<div class="col-md-1">
			<a style="margin-top: 24px; width:100%;" id="busqueda-seguimientoeh" class="btn btn-default"><span class="glyphicon glyphicon-search"></span></a>
		</div>
...
<script>
	...
	$(document).on("click", "#busqueda-seguimientoeh", function(event){

		event.preventDefault();
		var values = $('#form-seguimientoeh').find("input[type='hidden'], :input:not(:hidden)").serialize();
		...
		$.ajax({
			cache: false,
			type: 'POST',
			url: 'functions/busqueda-seguimientoeh.php',
			data: values,
