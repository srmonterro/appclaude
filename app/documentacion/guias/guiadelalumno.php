<?
    ...
    //require_once($baseurl.'/plugins/PHPMailer/SMTP.php');
    //require_once($baseurl.'/plugins/PHPMailer/Exception.php');
    ...
    if (isset($_POST[id_alumno]) || isset($_GET[id_alumno])) {
        ...
        CONCAT(al.nombre," ", al.apellido," ", al.apellido2) as alumno, e.razonsocial, a.objetivos, al.email as emailalumno, d.telefono, m.diascheck, e.agente, d.*, m.observaciones, e.cif
    ...
    while ($row = mysqli_fetch_array($sql)) {
        ...
        $agente = $row[agente];
        $cif_empresa = $row[cif]; // NUEVA VARIABLE PARA EL CIF
    ...
    // DEFINIR EL PORCENTAJE DE TIEMPO DE CONEXIÓN SEGÚN EL CIF
    $tiempo_conexion_porcentaje = ($cif_empresa == 'B35081090') ? '50%' : '30%';
   ...
   <page backleft="40px" backright="40px" backtop="140px" backbottom="60px" pagegroup="new">
    ...
    <page backleft="40px" backright="40px" backtop="140px" backbottom="60px">
        ...
        <page_footer>
            ...
            <ol class="listaindice">
                ...
                <li>Tiempo de conexión al curso: mínimo <? echo $tiempo_conexion_porcentaje; ?> de la duración total del curso. </li>
    ...
    $content = ob_get_clean();
        ...
        if ( !isset($_POST[enviomail]) )
            ...
            $cc = "-";
            $para = $emailalumno;
            // Definimos los datos de conexión al servidor SMTP
            $mail = new PHPMailer();
            $mail->isSMTP();
            ...
            $mail->SMTPSecure = 'tls';
            $mail->AuthType = 'LOGIN';
            $mail->Username = 'soporte@eduka-te.com';
            $mail->Password = 'K)989308310005at';

            $mail->setFrom('soporte@eduka-te.com', 'SOPORTE EDUKA-TE SOLUTIONS');
            $mail->addAddress($emailalumno);
            //$mail->addCC('monterro@gmail.com');
            ...
            if ($url == "http://campus2.eduka-te.com/") {
                ...
                URL:  http://campus.eduka-te.com/ y a continuación, el usuario y contraseña de los que ya disponéis.<br><br>';
                }
                $mail->Body .= '
                ...
                // DEFINIR EL PORCENTAJE PARA EL EMAIL SEGÚN EL CIF
                $tiempo_conexion_texto = ($cif_empresa == 'B35081090') ? '50%' : '30%';

                $mail->Body    .= '</table>
                <span style="color:#87BE5E"><strong>IMPORTANTE: El tiempo mínimo de conexión para obtener la calificación de APTO ha de ser, cómo mínimo, el '.$tiempo_conexion_texto.' de la duración de la acción formativa.</strong></span><br><br>
                Un saludo.<br>
