<?
    ...
    // if ( $modalidad == 'Mixta') {
    //     if ( $tipo == 'bonificado' ) {
    //         $q = 'UPDATE temp_alumnos SET user = '
    ...
    if ( $mod == 'p' )
        $mixto = ' AND md.mixto IN("od") ';
    else
        $mixto = ' ';

    $sql = 'SELECT DISTINCT a.numeroaccion, a.denominacion, a.horastotales, a.modalidad, ga.ngrupo,
        a.contenido, m.fechaini, m.fechafin, m.horariomini, m.horariomfin, m.horariotini, m.horariotfin, a.mixta, m.*,
        CONCAT(d.nombre," ",d.apellido," ",d.apellido2) as docente, d.email2 as emailtutor, d.email as emailcorp, a.url, a.objetivos, d.telefono, m.estado,m.diascheck, d.tipodoc,d.*
    ...
    while ($row = mysqli_fetch_array($sql)) {
        ...
        if ( $modalidad != "Mixta" ) {
            $fechainiphp = ($row[fechaini]);
            $fechafinphp = ($row[fechafin]);
            ...
            $fechainiphp = ($row[fechaini_nop]);
            $fechafinphp = ($row[fechafin_nop]);

            $fechaini = date("d/m/Y",strtotime($row[fechaini_nop]));
            $fechafin = date("d/m/Y",strtotime($row[fechafin_nop]));
            $horariomini = $row[horariomini_nop];
            $horariomfin = $row[horariomfin_nop];
            $horariotini = $row[horariotini_nop];
            $horariotfin = $row[horariotfin_nop];
            $diascheck = $row[diascheckod];
        }
    ...
    if ( $modalidad == "Mixta" ) {
        ...
        $fechasmt = array();
        $fechasmt = createDateRangeArray($fechainiphp,$fechafinphp);
        ...
        $sql = 'SELECT DISTINCT fecha
            FROM fechas_excluir
            WHERE id_matricula ='.$id_mat.' AND tipo = "od"';
            // echo $sql;
        ...
        $q3 = 'SELECT dia, horariomini, horariomfin, horariotini, horariotfin
            ...
            while ( $rowss = mysqli_fetch_array($q3) ) {
                ...
                $fechasIncluir[$z][horariomini] = $horariomini;
                $fechasIncluir[$z][horariomfin] = $horariomfin;
                $fechasIncluir[$z][horariotini] = $horariotini;
                $fechasIncluir[$z][horariotfin] = $horariotfin;
                $z++;
            }
    ...
    $guion = ' - ';
    if ( $diascheck == "LMXJV" )
        $dias = "Lunes a Viernes de ";
    else {
        for ($i=0; $i < strlen($diascheck); $i++) {

            if ( $i == strlen($diascheck)-1 )
                $guion = '';
            $dias .= $diascheck[$i].$guion.' de ';
        }
    }

    if ( count($fechasIncluir) > 0 || count($fechas_excluir) > 0 ) {
        $coma = ', ';
        for ($i=0; $i < count($fechasFinal); $i++) {

            if ( $i == count($fechasFinal)-1) $coma = "";
            $strfechas .= formateaFecha($fechasFinal[$i][dia]).$coma;

        }
        $dias = "";

    } else {
        $strfechas = $fechaini. ' - '.$fechafin;
    }
    ...
    if ( $tipo == 'bonificado' ) {
        ...
        if ( $estado == 'Finalizada' || $estado == 'Facturada' || $mod != 'p' )
            $q = 'SELECT DISTINCT a.*, e.*, a.email as emailalumno
            ...
            $q = 'SELECT *, a.email as emailalumno FROM '.$tablas.'
    ...
    // $id = 'idspring';
    // echo $q;
    $q = mysqli_query($link, $q) or die("error con alus");
    ...
    $nombreFichero = "GuiaDelAlumno_".$naccion."-".$ngrupo."_".str_replace(' ', '-', quitaTildes($alumno)).".pdf";
    // echo $nombreFichero;
    ob_start();
...
<?
    ...
    if ($id == "idspring") { ?>
    <page backimg="img/guia-manipulador-spring.png" backimgy="25px">

        <p class="letraspring" style="margin: 225 70"><? echo $alumno ?></p>
        <p class="letraspring" style="margin: -3px 497px "><? echo $fechaLarga ?></p>
        <p class="letraspring" style="margin: 555px 415px ">El trabajador/a a fecha <? echo $fechaLarga ?></p>
    ...
    <page backleft="40px" backright="40px" backtop="140px" backbottom="60px">
        ...
        <h3 style="margin-top: 30px" class="tituloseccion">1. Datos del Curso</h3>
            <table class="listadatos">
                ...
                <tr><td>Fechas: </td><td style="font-weight: bold"><? echo $strfechas ?></td></tr>
                ...
                <tr><td>Email tutor: </td><td style="font-weight: bold"><? echo $email ?></td></tr>
                <tr><td>Teléfono: </td><td style="font-weight: bold"><? echo $tlftutor ?></td></tr>
                <tr><td>Horario de Tutorías: </td><td style="font-weight: bold"><? echo $dias.$horario ?></td></tr>
    ...
    <page backleft="40px" backright="40px" backtop="140px" backbottom="60px" pagegroup="new">
        ...
        <page_footer>
            <div style="left:0px; "><img style="margin-bottom: -35px; width:150px" src="img/ISO9001_14001color.png" alt=""></div>
            <div class="circulo">
    ...
    <page backleft="40px" backright="40px" backtop="140px" backbottom="60px">
        ...
        <page_footer>
            ...
            <p>Para obtener la calificación de APTO debe recorrer y visualizar al menos el 75% del temario (contenido interactivo)  y responder correctamente un mínimo del 50% del total de las preguntas establecidas en los cuestionarios.
            Asimismo, el tiempo mínimo de conexión para obtener la calificación de APTO, ha de ser cómo mínimo el 25% de la duración de la acción formativa.</p>
            <p>No cumplir con estos requisitos implicará la NO superación del curso y la NO obtención
            del certificado de participación.</p>
    ...
    <? }
        ...
        if ( !isset($_GET[enviomail]) ) {
           ...
           //$mail->addAddress('aperojo@eduka-te.com');               // Name is optional
            $mail->addReplyTo('ysuarez@eduka-te.com');
            $mail->addBCC('ysuarez@eduka-te.com');
            $mail->addReplyTo('ctosco@eduka-te.com');
            $mail->addBCC('ctosco@eduka-te.com');
            $mail->addReplyTo('asantana@eduka-te.com');
            $mail->addBCC('asantana@eduka-te.com');
...
// $zip = create_zip($rutas_pdfs, 'pdf/GuiasMixto_'.$naccion.'-'.$ngrupo.'.zip');


// if ( $zip === true ) {
//     echo "OK";
//     // or however you get the path
//     // $yourfile = 'http://gestion.esfocc.com/app/documentacion/guias/pdf/GuiasMixto_'.$naccion.'-'.$ngrupo.'.zip';
//     // echo '<script type="text/javascript" language="javascript">
//     // window.open("'. $yourfile .'");
//     // </script>';
