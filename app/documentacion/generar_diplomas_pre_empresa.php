...
//ATAJO PARA IMPRIMIR DIPLOMAS DE OTROS AÑOS
//$gestion='2022';
...
$sql = 'SELECT DISTINCT a.numeroaccion, a.denominacion, a.horastotales, a.modalidad, ga.ngrupo, a.contenido,
    m.fechaini, m.fechafin, m.horariomini, m.horariomfin, m.horariotini, m.horariotfin, m.estado, a.incendios, a.nivel_incendios, a.diploma, m.referencia_legio, m.fechaini_nop, m.fechafin_nop
...
if ( $gestion == 2014 ) {
    ...
    while ($row = mysqli_fetch_array($sql)) {
        ...
        <? if ( strpos($_SERVER['HTTP_REFERER'], 'form_matricula_doc') ) { ?>
        <div class=<? echo '"'. $back .'"' ?>>
    ...
    if ( $incendios == 1 ) {
        ...
        if ( $nivel_incendios == "I" ) $clase = "front_bonif";
    ...
    if ( $estado == 'Finalizada' || $estado == 'Facturada' || $estado == 'Gratuita' || $grupal == 1 )
        ...
        $sql = 'SELECT DISTINCT a.id, a.nombre,a.apellido,a.apellido2,a.documento,e.razonsocial,e.cif, e.grupo
        FROM mat_alu_cta_emp ma, alumnos a, empresas e
        WHERE ma.id_alumno = a.id
        AND ma.finalizado = 1
        AND ma.tipo = ""
        ...
        $sql = 'SELECT a.id, a.nombre,a.apellido,a.apellido2,a.documento,e.razonsocial,e.cif, e.grupo
        FROM temp_alumnos a,temp_empresas e
        WHERE a.id_empresa = e.id
        AND e.id = '.$id_emp.'
    ...
    while ($row = mysqli_fetch_array($sql)) {
        ...
        <? //if ( strpos($_SERVER['HTTP_REFERER'], 'form_matricula_doc') || $row[grupo] == 1 ) { ?>
        <div class=<? echo '"'. $back .'"' ?>>
            <div class="contenidotxt">
                <div><? echo nl2br($contenido) ?></div>
            </div>
