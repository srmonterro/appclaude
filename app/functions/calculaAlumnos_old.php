...
for ($i=0; $i < 12; $i++) {
	...
	AND m.fechafin >= "2022-'.$j.'-01" AND m.fechafin <= "2022-'.$j.'-31"';
			// echo $q;
	$q = mysqli_query($link2014, $q) or die("error" . mysqli_error($link2014));
	$row = mysqli_fetch_array($q);

			// echo $row[alutotal];
	$alus2014[$i] = $row[alutotal];
	...
	AND m.estado IN ("Finalizada", "Facturada","Liquidada", "Gratuita")
	AND m.estado NOT IN ("Anulada")
	AND m.fechafin >= "2019-'.$j.'-01" AND m.fechafin <= "2019-'.$j.'-31"';
			// echo $qx;
	$qx = mysqli_query($link2014, $qx) or die("error" . mysqli_error($link2014));
	$rowx = mysqli_fetch_array($qx);

	$mat2014[$i] = $rowx[mattotal];

	$qy = 'SELECT DISTINCT SUM(fb.total_factura) as facttotal
	FROM facturacion_bonificada fb
	WHERE fb.fecha >= "2019-'.$j.'-01" AND fb.fecha <= "2019-'.$j.'-31"
	UNION
	SELECT DISTINCT SUM(fb.total_factura) as facttotal
	FROM facturacion_privada fb
	WHERE fb.fecha >= "2019-'.$j.'-01" AND fb.fecha <= "2019-'.$j.'-31"';
			// echo $qy;
	$qy = mysqli_query($link2014, $qy) or die("error" . mysqli_error($link2014));
			// $rowy = mysqli_fetch_array($qy);

	$suma = 0;
	while ($rowy = mysqli_fetch_array($qy)) {
		if ( $rowy[facttotal] == null ) $rowy[facttotal] = 0;
		$suma = $rowy[facttotal]+$suma;
	}

	$fac2014[$i] = round($suma, 2);
