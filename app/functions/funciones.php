...
function devuelveAlumnosGuias($id_mat, $tipo, $mod, $link) {
    ...
    // $tablausers = '<table><tr><td>username</td><td>password</td><td>firstname</td><td>lastname</td><td>email</td><td>phone</td><td>course1</td><td>group1</td><td>city</td><td>country</td></tr>';
    $tablausers = '<table><tr><td>username</td><td>password</td><td>firstname</td><td>lastname</td><td>email</td><td>phone</td><td>profile_field_dni</td><td>profile_field_empresa</td><td>profile_field_cif</td><td>course1</td><td>group1</td><td>city</td><td>country</td></tr>';
    ...
    while ( $rows = mysqli_fetch_array($q) ) {
        ...
        <div style="margin: 15px 0px 20px 0px; font-size:12px; overflow: auto" class="col-md-12">
            ...
            <div class="col-md-1" style="margin-top: 25px">
                <a style="width:100%" id="guardauserbon_grupal" user="'.$i.'" name="'.$rows[id_alumno].'" tipo="'.$tipo.'" class="btn btn-success"><span class="glyphicon glyphicon-ok-circle"></span></a>
                </div>
            <div class="col-md-1" style="margin-top: 25px">
        ...
        $tablausers .= '<tr><td>'.$user.'</td><td>'.$pass.'</td><td>'.$rows[nombre].'</td><td>'.$rows[apellido].' '.$rows[apellido2].'</td><td>'.$email.'</td><td>'.$phone.'</td><td>'.$rows[documento].'</td><td>'.$rows[razonsocial].'</td><td>'.$rows[cif].'</td><td>'.$naccion.$denominacion.'</td><td>'.$naccion.'.'.$ngrupo.'</td><td>Islas Canarias</td><td>ES</td></tr>';
...
function mostrarTabla ($tabla,$mat,$link,$id) {
    ...
    if ( $tabla == 'docentes' ) {
        ...
        if ( $sec == 'index.php?presencial' )
            ...
            $q1 = 'SELECT  m.id, m.fechaini, m.fechafin, m.tipo_docente, a.numeroaccion, a.denominacion, ga.ngrupo, m.estado, m.solicitud
        FROM matriculas m, acciones a, grupos_acciones ga
        WHERE m.id_accion = a.id
        ...
        LIMIT 0,30';
            ...
            $q1 = 'SELECT  m.id, m.fechaini, m.fechafin, m.tipo_docente, a.numeroaccion, a.denominacion, ga.ngrupo, m.estado
        FROM matriculas m, acciones a, grupos_acciones ga
        WHERE m.id_accion = a.id
        AND ga.id = m.id_grupo
    ...
    ORDER BY id DESC';
        ...
        if ( $_SESSION['user'] == 'margarita' || $_SESSION['user'] == 'root' || $_SESSION['user'] == 'alago' ) {
            ...
            $q1 = 'SELECT  m.id, m.fechaini, m.fechafin, m.tipo_docente, a.numeroaccion, a.denominacion, ga.ngrupo, m.estado, ma.progreso, ma.finalizado,
            al.nombre, al.apellido, al.id as id_alu, a.modalidad, a.url, ma.tipo
            FROM matriculas m, acciones a, grupos_acciones ga, docentes d, mat_doc md, usuarios u, mat_alu_cta_emp ma, alumnos al
            WHERE m.id_accion = a.id
            AND ga.id = m.id_grupo
            ...
            AND u.user = "'.$_SESSION['user'].'"
            UNION
            SELECT  m.id, m.fechaini, m.fechafin, m.tipo_docente, a.numeroaccion, a.denominacion, ga.ngrupo, m.estado, ma.progreso, ma.finalizado,
            al.nombre, al.apellido, al.id as id_alu, a.modalidad, a.url, ma.tipo
            FROM matriculas m, acciones a, grupos_acciones ga, docentes d, mat_doc md, usuarios u, mat_alu_cta_emp ma, alumnos al
            WHERE m.id_accion = a.id
            AND ga.id = m.id_grupo
            ...
            AND u.user = "'.$_SESSION['user'].'"
            UNION
            SELECT  m.id, m.fechaini_nop, m.fechafin_nop, m.tipo_docente, a.numeroaccion, a.denominacion, ga.ngrupo, m.estado,
            al.nombre, al.apellido, al.id as id_alu, a.modalidad, a.url, ma.tipo
            FROM matriculas m, acciones a, grupos_acciones ga, docentes d, mat_doc md, usuarios u, mat_alu_cta_emp ma, alumnos al
            WHERE m.id_accion = a.id
            AND ga.id = m.id_grupo
    ...
    AND m.grupo = 0
    UNION
    SELECT  m.id, m.fechaini, m.fechafin, m.tipo_docente, a.numeroaccion, a.denominacion, ga.ngrupo, m.estado,
    al.nombre, al.apellido, solicitud
    FROM matriculas m, acciones a, grupos_acciones ga, alumnos al, mat_alu_cta_emp ma
    WHERE m.id_accion = a.id
    AND ga.id = m.id_grupo
        ...
        <table id="tablamatriculas" class="table table-striped">
            ...
            // echo $row[solicitud];
                    ...
                    if ( $sec == 'index.php?matricula' ) {
                        ...
                        print($row[nombre].' '.$row[apellido]);
                        echo "</td>"; }
                        echo "<td id='tipo_docente'>";
                        print($row[tipo_docente]);
                        echo "</td>";
                        echo "<td id='estado'>";
...
function envioMailIKEA ($numeroaccion, $ngrupo, $tipo, $link, $opt=NULL) {
    ...
    // Para enviar un correo HTML mail, la cabecera Content-type debe fijarse
            ...
            // break;
                ...
                $cc = 'shirley.gonzalez@eduka-te.com';
        ...
        $para = 'margarita.mitkova@eduka-te.com';
        $cc = 'backup-gestion@eduka-te.com, ivan.cabrera@eduka-te.com, shirley.gonzalez@eduka-te.com';                // $para = 'ivan.cabrera@eduka-te.com';
...
//
    ...
    $para = 'margarita.mitkova@eduka-te.com';
    $cc = 'margarita.mitkova@eduka-te.com, shirley.gonzalez@eduka-te.com, ivan.cabrera@eduka-te.com';
                // $para = 'ivan.cabrera@eduka-te.com';
    ...
    $url = explode('.com/', $row[url]);
            ...
            $para = 'ivan.cabrera@eduka-te.com';
            $cc = 'margarita.mitkova@eduka-te.com,shirley.gonzalez@eduka-te.com';
            ...
            $para       = 'margarita.mitkova@eduka-te.com';
            //$para       = 'mchinea@eduka-te.com';
            $cc = 'ivan.cabrera@eduka-te.com,shirley.gonzalez@eduka-te.com';
