 ...
 foreach ($_POST as $key => $value) {
    ...
    if ($todo == 1)
        ...
        $union = ' UNION
                        ...
                        f.cobrado, f.base_facturacion, f.igic, f.total_factura, f.importe_a_abonar, f.estado, f.prefijo, f.numero, f.fecha, f.id as idfac, f.fecha_vencimiento, c.nombre as nombrecomercial, f.facturar_a, e.formapago, f.IKEA, f.observacionesfra, f.costes_organizacion, cc.fecha as fechacc, cc.observaciones
                        FROM acciones ac, empresas e, grupos_acciones ga, matriculas m, '.$tabla1.' f, comerciales c, grupos_empresas g, conciliaciones cc
                        ...
                        GROUP BY e.id,m.id,numero
                         UNION
                        SELECT @idmat:=m.id as mat, ac.modalidad, e.razonsocial, ac.numeroaccion, ga.ngrupo, m.fechaini, m.fechafin, ac.denominacion, e.cif, e.iban,
                        f.cobrado, f.base_facturacion, f.igic, f.total_factura, f.importe_a_abonar, f.estado, f.prefijo, f.numero, f.fecha, f.id as idfac, f.fecha_vencimiento, c.nombre as nombrecomercial, f.facturar_a, e.formapago, f.IKEA, f.observacionesfra, "" as costes_organizacion, cc.fecha as fechacc, cc.observaciones
                        FROM acciones ac, empresas e, grupos_acciones ga, matriculas m, '.$tabla2.' f, comerciales c, grupos_empresas g, conciliaciones cc
                        WHERE ac.id = m.id_accion
                        '.$comercial.'
                        AND ga.id = m.id_grupo
                        AND f.numero=cc.factura
    ...
    if($tabla != 'facturacion_rectificativa'){
        $q = 'SELECT @idmat:=m.id as mat, ac.modalidad, e.razonsocial, ac.numeroaccion, ga.ngrupo, m.fechaini, m.fechafin, ac.denominacion, e.cif, e.iban,
            f.cobrado, f.base_facturacion, f.igic, f.total_factura, f.importe_a_abonar, f.estado, f.prefijo, f.numero, f.fecha, f.id as idfac, f.fecha_vencimiento, c.nombre as nombrecomercial, f.facturar_a, e.formapago, f.IKEA, f.observacionesfra, f.costes_organizacion, cc.fecha as fechacc, cc.observaciones
            FROM acciones ac, empresas e, grupos_acciones ga, matriculas m, '.$tabla.' f, comerciales c, grupos_empresas g,  conciliaciones cc
            WHERE ac.id = m.id_accion
            '.$comercial.'
            AND ga.id = m.id_grupo
            AND f.numero=cc.factura
    ...
    while ( $row = mysqli_fetch_assoc($q) ) {
        ...
        echo '<td id="numcuenta" style="text-align: center">'.formateaFecha($row[fechacc]).'</td>';
